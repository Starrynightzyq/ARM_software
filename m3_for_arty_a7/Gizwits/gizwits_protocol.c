...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(0 != memcmp((uint8_t *)&last->valueCar_read,(uint8_t *)&cur->valueCar_read,sizeof(last->valueCar_read)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCar_read Changed");
            gizwitsProtocol.waitReportDatapointFlag.flagCar_read = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagCar_read)
    {
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)dataPoints->valueCar_read,sizeof(dataPoints->valueCar_read));
        byteFieldOffset += sizeof(dataPoints->valueCar_read);
    }
